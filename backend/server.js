...
app.post('/api/guides/:id/reset-password', auth, async (req, res) => {
  ...
  try {
    const result = await integrationService.resetGuidePassword(
      req.params.id,
      password,
      req.user,
    );
...
app.put('/api/guides/:id', auth, async (req, res) => {
  if (!ensureFirebaseDataMode(res)) return;

  try {
    const updated = await integrationService.updateGuide(
      req.params.id,
      req.body || {},
      req.user,
    );
...
app.get('/api/rewards', auth, async (req, res) => {
  ...
  try {
    const rewards = await integrationService.getRewards(req.user);
...
app.post('/api/rewards', auth, async (req, res) => {
  ...
  try {
    const reward = await integrationService.createReward(req.body || {}, req.user);
...
app.put('/api/rewards/:id', auth, async (req, res) => {
  ...
  try {
    const reward = await integrationService.updateReward(
      req.params.id,
      req.body || {},
      req.user,
    );
...
app.patch('/api/rewards/:id/status', auth, async (req, res) => {
  ...
  try {
    const reward = await integrationService.setRewardStatus(
      req.params.id,
      status,
      req.user,
    );
...
app.delete('/api/rewards/:id', auth, async (req, res) => {
  ...
  try {
    await integrationService.deleteReward(req.params.id, req.user);
    return res.json({ success: true });
  } catch (error) {
    if (error.message === 'Forbidden') {
      return res.status(403).json({ message: error.message });
